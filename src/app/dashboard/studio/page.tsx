...
const emptyKit: Kit = { product: {}, en: {}, bn: {} }
...
export default function Studio() {
  ...
  async function upload(file: File) {
    ...
    try {
      ...
      setImage(s.storage.from('product-images').getPublicUrl(path).data.publicUrl)
  ...
  async function generate() {
    ...
    try {
      ...
      setKit({ ...emptyKit, ...d.content }); setTab('captions')
  ...
  return <div className="mx-auto max-w-7xl">
    <div className="mb-7 flex flex-wrap items-end justify-between gap-4">
      <div><p className="text-xs font-black uppercase tracking-[0.2em] text-teal">AI PRODUCT MARKETING STUDIO</p><h1 className="mt-2 text-4xl font-black tracking-tight">One photo. A complete sales kit.</h1><p className="mt-2 max-w-2xl text-brand/55">Generate sales captions, trustworthy specifications, hooks, CTAs and reel scripts in English and Bangla.</p></div>
      <button onClick={generate} disabled={busy} className="rounded-2xl bg-teal px-6 py-3 font-black text-white shadow-sm disabled:opacity-50">{busy ? '✨ Generating…' : '✨ Generate marketing kit'}</button>
    </div>

    <div className="grid gap-6 xl:grid-cols-[360px_1fr]">
      <section className="rounded-3xl border border-brand/10 bg-white p-5 shadow-sm">
        <div className="rounded-2xl border-2 border-dashed border-brand/10 bg-beige/60 p-3">
          <label className="block cursor-pointer text-center"><input type="file" accept="image/png,image/jpeg,image/webp" className="hidden" onChange={e => { const f=e.target.files?.[0]; if(f) upload(f) }} />
            <div className="flex min-h-56 items-center justify-center overflow-hidden rounded-xl bg-sky/40">{image ? <img src={image} alt="Product" className="h-56 w-full object-contain"/> : <div><div className="text-5xl">📸</div><p className="mt-3 font-black">Upload product photo</p><p className="mt-1 text-xs text-brand/45">JPG, PNG or WEBP · max 8 MB</p></div>}</div>
          </label>
        </div>
